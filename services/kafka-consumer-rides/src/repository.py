"""
Kafka Consumer Rides repository — in-memory archive simulation for MinIO Bronze layer.
"""
...
class RideArchiveRepository:
    """In-memory archive storage simulating MinIO Bronze layer writes."""

    def __init__(self, bucket: str = "bronze", prefix: str = "kafka/ride.events.v1"):
        self.bucket = bucket
        self.prefix = prefix
    ...
    def _generate_file_path(self, topic: str, timestamp: datetime) -> str:
        """Generate MinIO path: kafka/ride.events.v1/year=YYYY/month=MM/day=DD/batch_HHMMSS.json.gz"""
        return (
            f"{self.prefix}/"
    ...
    def archive_batch(self, events: list[dict], topic: str = "ride.events.v1") -> tuple[str, int]:
        """
        Archive a batch of events as a compressed JSON file.
