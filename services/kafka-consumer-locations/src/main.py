"""
Kafka Consumer Locations — FastAPI application.

Archives location events to MinIO Bronze layer as compressed JSON files.

ROUTES:
  POST /archive          — Archive a batch of location events to MinIO Bronze
...
_repo = repository.LocationArchiveRepository(
    bucket=service_config.settings.archive_bucket,
    prefix=service_config.settings.archive_prefix,
)


def get_repo() -> repository.LocationArchiveRepository:
    """Dependency — returns the location archive repository."""
    return _repo
...
app = create_app(
    title="Kafka Consumer Locations",
    version="0.1.0",
    description="Archives location events to MinIO Bronze layer as compressed JSON",
    lifespan=lifespan,
)
...
async def archive_batch(
    ...
    repo: repository.LocationArchiveRepository = Depends(get_repo),
):
    """Archive a batch of location events to MinIO Bronze layer."""
